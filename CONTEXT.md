...
- Từ M5 (nhánh `m5`, chưa merge): signaling P2P **chỉ** chuyển giữa hai bên đã bind của một relay đã được receiver chấp nhận, tới đúng đối tác, có cap size + rate limit + TTL; host không đọc nội dung và không nhận payload P2P. Payload P2P được WebRTC mã hoá bằng DTLS, nhưng **signaling thì không** — SDP/candidate đi trên WS plaintext trong LAN. Chi tiết: `docs/adr/0005-p2p-transport-authority.md`.
