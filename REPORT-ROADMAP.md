...
Bằng chứng gate cuối lượt M1: sau khi khắc phục các lỗi review vòng 1 và vòng 2 (R1–R7), `npm run quality` pass trên Node / Linux — 207/207 automated tests pass, lint và format sạch, coverage 93,59% line / 88,53% branch / 93,49% function. Kiểm chứng browser thủ công bằng headless Chrome (CDP): cổng PIN hiện khi bật PIN, sai PIN báo lỗi và không lưu token, đúng PIN mở được danh sách file đã stage, bốn view render không lỗi console. Đã thêm regression test suite tại seam HTTP/WS thật khóa chặt R1–R7, timeout/never-settling child process handling, và junction fallback cho Windows. Chưa chạy Node 24, Windows, CI, điện thoại thật.
...
Năm mục `done (M1)` ở trên đã qua acceptance criteria với test tại seam thật, nhưng giới hạn kiểm chứng vẫn giữ: chỉ Node 22/Linux, chưa có CI, chưa test trên điện thoại thật, chưa có TLS. Chi tiết trong mục 2 và ADR-0002.
...
| M1 — Quyền và runtime     | UT-002, UT-003, UT-005, UT-010, UT-015 — đã triển khai cùng lượt với UT-001 (xem mục 2).                                       | Negative auth tests pass (đã có); host/client role thực (đã có); QR/runtime đúng (đã có); start/stop không đụng process khác (đã có, còn thiếu smoke trên Windows). |
