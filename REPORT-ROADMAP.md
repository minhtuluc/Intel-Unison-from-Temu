...
| UT-007 | P1 / in-progress (M2) | Chunk manager không đối chiếu byte count thực tế; max session/devices/concurrency config chưa enforce.                                                         | Đang xử lý theo QC Review R2: tách auth session vs upload session; StorageQuotaTracker byte quota; validate integer size/checksum; enforce maxConnectedDevices và maxConcurrentTransfers.                            |
...
Các mục M2 (UT-004, UT-006, UT-007, UT-008, UT-009, UT-013, UT-014, UT-017) đang được cập nhật và bổ sung regression suite giải quyết toàn bộ 4 điểm yêu cầu thay đổi (R1–R4) từ QC review (`REVIEW-M2.md`).
...
| M2 — Truyền file tin cậy  | UT-004, UT-006–009, UT-013–014, UT-017 — đang xử lý QC review R1–R4 trên nhánh `m2-reliable-transfer`.                         | Giải quyết triệt để R1 (complete idempotency/locking), R2 (resource limits/quota/checksum), R3 (accept/decline CAS state machine), R4 (WS routing/event pruning).     |
