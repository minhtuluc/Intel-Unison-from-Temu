...
| UT-007 | P1 / in-progress (M2) | Chunk manager không đối chiếu byte count thực tế; max session/devices/concurrency config chưa enforce.                                                         | Đang xử lý theo QC Review R2/R5/R6/R9: memory-bounded incremental SHA-256; pre-claim concurrency slot; startup disk quota reconcile & multipart rollback; strict 64-hex checksum validation.                                                                       |
...
Các mục M2 (UT-004, UT-006, UT-007, UT-008, UT-009, UT-013, UT-014, UT-017) đã giải quyết toàn bộ 12 điểm yêu cầu từ QC review (R1–R4 vòng 1, R5–R10 vòng 2, và R11–R12 vòng 3 trong `REVIEW-M2.md`).
...
| M2 — Truyền file tin cậy  | UT-004, UT-006–009, UT-013–014, UT-017 — hoàn thành giải quyết QC review R1–R12 trên nhánh `m2-reliable-transfer`.             | Hoàn thành giải quyết triệt để R1–R4 (vòng 1), R5–R10 (vòng 2), và R11–R12 (vòng 3: atomic reservation `SimpleUploadQuotaStorage` chống TOCTOU collision, và cross-session connectionId spoofing defense trên cùng IP có PIN). |
